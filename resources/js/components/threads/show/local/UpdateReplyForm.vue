<template>
  <div class="flex flex-row w-full">
    ...
    >
      ...
      <button type="submit" @click.prevent="changeDisplay">Düzenle</button>
    </div>
    <div class="flex flex-1 flex-col" v-show="displayForms">
      <form @submit.prevent="updateReply()" class="flex flex-1 flex-col">
        <textarea
          required
  ...
  methods: {
    updateReply() {
      this.form
        ...
        .catch((error) => {
          flash(
            error.errors.body[0] ? error.errors.body[0] : error.message,
            "error"
          );
        });
    },
